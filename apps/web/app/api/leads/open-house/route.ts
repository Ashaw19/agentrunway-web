...
export async function POST(request: NextRequest) {
  ...
  const formUrl = typeof body.formUrl === "string" ? body.formUrl : "/tools/open-house-signup";
  ...
  if (submissionError) {
    console.error("[leads/open-house] ✗ insert error:", submissionError.message);
    return NextResponse.json(
  ...
  if (consentError) {
    console.error("[leads/open-house] ✗ consent insert error:", consentError.message);
  }
  ...
  if (resend) {
    try {
      await resend.emails.send({
        ...
        subject: `New open house signup — ${firstName} ${lastName}`,
        html: `
          <p><strong>New open house signup via Agent Runway</strong></p>
        ...
        `,
        text: `New open house signup\n\nName: ${firstName} ${lastName}\nEmail: ${email}${phone ? `\nPhone: ${phone}` : ""}${propertyAddress ? `\nProperty: ${propertyAddress}` : ""}${message ? `\nMessage: ${message}` : ""}\n\nConsent recorded. IP: ${ip}`,
      });
    } catch (err) {
      console.error("[leads/open-house] ✗ notification email failed:", err instanceof Error ? err.message : err);
    }
  }
