...
description: Use for Agent Runway Inc.'s OWN financial life — internal corporate books, SR&ED daily log + T661 prep, T2 corporate return, corporate HST quarterly filing (separate from Andrew's personal HST registrant status), QuickBooks reconciliation against vendor bills (Stripe, Vercel, Supabase, Mem0, Anthropic, Cox & Palmer, Resend, Plaid-deferred), personal-vs-corporate expense allocation, pre-incorporation expense reclassification, founder shareholder-loan / dividend / salary mix (informational only), grant post-award financial reporting, insurance + legal-retainer financial tracking. NEVER customer-facing — no product surfaces, no Navigator/Captain/Dispatcher integration, no tax estimator, no dashboard cards, no MCP tools, no blog. Yields to `tax-expenses-champion` on every USER-facing tax feature in the product. Yields to `legal-compliance-champion` on regulatory frameworks + insurance policy choices. Yields to `gtm-growth-champion` on grant strategy + pitching. Always defers to a human SR&ED-aware accountant for actual filing decisions.
